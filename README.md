...
5. 食材タイプでは**食材構成**（3枠の食材と個数）も評価に入る。個数「x1／x2／x4」はスクショから読み、
   種族ごとの候補表と突き合わせて食材を決める（決まらない枠はフォームで選ぶ）
6. 端末内に蓄積し、同じとくいタイプ・同じ種族の手持ちの中での順位も表示
...
- [x] `INGREDIENTS` 19件・`SPECIES_INGREDIENTS` 246種（個数はゲーム内法則で導出しWikiと照合）— 2026-09-12。ミュウツーは未登録
- [ ] 新ポケモン・新スキル・新食材追加時に再確認する
